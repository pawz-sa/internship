...
const MenuButton = ({ iconType, label, onClick, className = "" }) => {
  const renderIcon = () => {
    switch (iconType) {
      case "home":
        return (
  ...
  return (
    <button
      onClick={onClick}
      className={`flex items-center pr-[15px] w-full left-[9px] top-[13px] hover:bg-[#505050] ${className}`}
    >
      {className.includes("text-[#ff5722]") && <span className="w-[4px] h-[30px] bg-[#FF5722] mr-[10px]" z-11></span>}
      <div className="w-6 h-6 flex items-center justify-center">{renderIcon()}</div>
      <span className="text-[17px] pb-[11px] pr-[48px] pl-[17px] pt-[12px] font-[monospace] font-bold leading-[25.5px]">
        {label}
      </span>
    </button>
  )
